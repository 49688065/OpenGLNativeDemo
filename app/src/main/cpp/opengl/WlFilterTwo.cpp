...
void WlFilterTwo::onCreate() {
    ...
    LOGD("opengl program is %d vShader %d fShader %d",program,vShader,fShader);
...
void WlFilterTwo::setPilex(void *data, int width, int height, int length) {
    ...
    h = height;
    pixels = data;
    LOGE("3. width %d height %d  surface_width %d  surface_height %d",width,height,surface_width,surface_height);
    if (surface_height >0 && surface_height > 0)
    {
...
void WlFilterTwo::destroySource() {
    if (pixels != NULL)
    {
        pixels= NULL;
    }
